...
mkdir -p ${ROOT_DIR}/docker-build-trixie/

docker pull debian:trixie-slim

docker run -t ${DOCKER_FLAGS} --rm \
    -v ${ROOT_DIR}:/source:ro \
    -v ${ROOT_DIR}/docker-build-trixie:/target:rw \
    debian:trixie-slim \
    bash /source/Resources/Builders/Debian/docker-internal.sh $1 trixie 3.13 $(id -u) $(id -g)

ls -lR ${ROOT_DIR}/docker-build-trixie/
